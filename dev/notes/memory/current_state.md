...
- **[2026-05-19, amended twice] Env-parity contract + unification plan + LOOP CAN REBUILD SHIFTER**: `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. `plans/active/env_install_unification_plan.md` is the destination spec — moves pip deps onto `pyproject.toml` extras (`[dev]`, `[full]`) as the single source of truth.
  ...
  **🛠 SHIFTER REBUILD PENDING (2026-05-19, after slice 6)**: `env_install_unification_plan` slices 2 + 3 + 4 + 5 + 6 + 7 have all SHIPPED. The Dockerfile now drives the container build through `pip install .[full]` instead of the per-sibling `<NAME>_SPEC` ARGs. **Build attempt history**:
   ...
   - **⭐ DIRECTIVE A (pscratch GraphRoot override, AUTHORIZED — overrides standing "No pscratch overlay" rule for THIS specific case)**: move podman storage to pscratch so the 40 GiB home quota stops gating builds. Loop steps:
     1. Reuse the existing scaffolded location `/pscratch/sd/a/adammwea/storage/` (already has `overlay/`, `overlay-images/`, `overlay-layers/` stub dirs — looks like a prior partial setup); OR pick a clean new location like `/pscratch/sd/a/adammwea/podman_storage/`. Either works.
     2. Write `~/.config/containers/storage.conf` setting `[storage] driver = "overlay"` + `graphroot = "<pscratch path>"`. Keep runroot at podman's default (tmpfs is fine).
     3. Verify with `podman info | grep -E "graphRoot|runRoot"` — graphroot should resolve to the pscratch location.
     4. Smoke-test with a tiny `podman pull` (e.g. `podman pull alpine`) to confirm new graphroot is writable. Then remove the test image (`podman rmi alpine`).
     5. Commit the change with `claude: shifter rebuild prep — move podman GraphRoot to pscratch (user-authorized override)`. Track in commit_log + memory.
     6. NOTE: standing "No pscratch overlay" rule remains in effect for ALL OTHER purposes — this override is scoped specifically to podman's image-build cache. Code stays in /global/homes; iteration outputs stay in /pscratch/.../dev_outputs/.

   - **⭐ DIRECTIVE B (slim the Dockerfile install to kill ~5 GiB of duplication)**: the `kilosort4-base:4.0.38_cuda-12.0.0` base image ALREADY contains torch + the entire `nvidia/*` CUDA wheel stack + triton (~5 GiB total). `pip install .[full]` reinstalls all of them as transitive deps of `UnitMatchPy @ git+...` (whose pyproject hard-deps `torch>=2.1,<3.0`). The result is a ~17-20 GiB image with ~5 GiB of waste. Loop must fix this BEFORE attempting the rebuild — otherwise the build is much bigger than necessary and a future re-rebuild is again at risk of running out of space. Recommended approach (loop can revise if it surveys the base image and finds a cleaner path):
     1. **Audit the base image's pre-installed packages**: spin a quick `podman run --rm docker.io/spikeinterface/kilosort4-base:4.0.38_cuda-12.0.0 pip list` (or `pip freeze`) to enumerate what's already there. Document in a new `dev/notes/refs/kilosort4_base_audit.md`.
     2. **Split `pyproject.toml` `[full]`** into `[full]` (current behavior — for laptop / non-CUDA-base installs; keeps torch + nvidia) and `[full-cuda]` (everything in `[full]` minus torch + nvidia/* + triton; assumes the host image already has them). The two extras share their non-torch deps via a list constant if possible to avoid drift.
     3. **Dockerfile swap**: change `RUN pip install .[full]` → `RUN pip install .[full-cuda]`.
     4. **Smoke-test the slim path locally**: `pip install -e .[full-cuda]` should succeed on a host that already has torch installed (the existing conda env qualifies). Tests should pass.
     5. **Then rebuild** the shifter image — should come in at ~12-14 GiB instead of ~17-20 GiB.
     6. Commit chain: `claude: pyproject — split [full] into [full] + [full-cuda]`, then `claude: Dockerfile — use [full-cuda] to avoid duplicating base-image CUDA stack`.
     - Alternative if (B.2) hits a snag: try constraint file (`pip install -c containers/base_constraints.txt .[full]` where the constraint file pins torch/nvidia to satisfied versions matching the base image), or `--no-deps UnitMatchPy` + list its non-torch deps directly. Pick whichever survives audit.

   - **Then rebuild**: `podman build` → `podman push docker.io/adammwea/axon-recon:pipeline-v2` → `shifterimg pull docker:adammwea/axon-recon:pipeline-v2` → in-container smoke (`shifter --image=adammwea/axon-recon:pipeline-v2 python -c "import UnitMatchPy.bayes_functions"`; kssynth + unitlink will remain ImportError until GH remotes are created and added to `[full]` per USER INJECTION #4).
   - **Login check**: as of 2026-05-19 16:55 PDT, `podman login --get-login docker.io` returns `adammwea` (logged in). Login is NOT the blocker.
