...
- **[2026-05-19, amended twice] Env-parity contract + unification plan + LOOP CAN REBUILD SHIFTER**: `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. `plans/active/env_install_unification_plan.md` is the destination spec — moves pip deps onto `pyproject.toml` extras (`[dev]`, `[full]`) as the single source of truth.
  ...
  **🛠 SHIFTER REBUILD PENDING (2026-05-19, after slice 6)**: `env_install_unification_plan` slices 2 + 3 + 4 + 5 + 6 + 7 have all SHIPPED. The Dockerfile now drives the container build through `pip install .[full]` instead of the per-sibling `<NAME>_SPEC` ARGs. The loop attempted the rebuild itself on **2026-05-19 16:55 PDT** but it FAILED:
   - `podman build -f containers/axon-recon/Dockerfile -t docker.io/adammwea/axon-recon:pipeline-v2 .` failed at base-image-pull step with `disk quota exceeded` writing scipy test data to home.
   - Root cause: home filesystem is at **117% of 40GiB quota** (`myquota` says SPACE_USED 46.83GiB). Podman's `GraphRoot` is `~/.local/share/containers/storage` (~9.3GiB) which competes with the rest of home for that 40GiB cap. Base image alone (`docker.io/spikeinterface/kilosort4-base:4.0.38_cuda-12.0.0`) is large; the layer extraction couldn't complete.
   - **🛑 BLOCKER FOR LOOP**: the loop CANNOT proceed with the rebuild until one of (a) home quota is bumped, (b) home is cleaned up to <40GiB, or (c) podman's GraphRoot is moved off home (e.g. to pscratch). The standing "Code lives in /global/homes. No pscratch overlay." constraint suggests (a) or (b) is the right move — (c) would put container storage on pscratch which conflicts with the directive. **User action required**: pick one of those three.
   - Once unblocked, the loop will re-attempt: `podman build` → `podman push docker.io/adammwea/axon-recon:pipeline-v2` → `shifterimg pull docker:adammwea/axon-recon:pipeline-v2` → in-container smoke (`shifter --image=adammwea/axon-recon:pipeline-v2 python -c "import UnitMatchPy.bayes_functions"`; kssynth + unitlink will remain ImportError until GH remotes are created and added to `[full]` per USER INJECTION #4).
   - **Login check**: as of 2026-05-19 16:55 PDT, `podman login --get-login docker.io` returns `adammwea` (logged in). The login is NOT the blocker; it's the disk quota.
