...
- **[2026-05-21] Phase-enable for tests (general rule)**: When a slice needs to smoke-test a phase that's currently YAML-disabled (`enabled: false`), the loop MUST enable the phase for the smoke — either temporarily via a YAML edit it reverts, or via a CLI override. Never test a phase while it's disabled and treat the no-op as validation. Per user 2026-05-21: "Feel free to enable phases as needed for tests. Don't test it disabled and think that validates it."
  - **Immediate application — kssynth_recon_integration slice 3a→3b sequencing**: Loop's next slice is a `--force-enable <phase>` CLI override (general, reusable; preferred over per-slice YAML edits). Spec: add a `--force-enable PHASE[,PHASE...]` flag to the reconstruct (and ideally other stage) CLIs in `pipeline/cli/` that flips matching phases to `enabled=True` AFTER YAML parsing but BEFORE phase-roster evaluation. Add a unit test + 1 dry-run smoke confirming the override works. Then proceed to slice 3b smoke of kssynth using the new flag (`--force-enable kssynth`).
  - **Promote when stable**: once the CLI flag is in place and used by ≥2 slices without issues, promote the "enable phases for tests" rule into `guardrails/` (probably alongside the slice-protocol smoke-test ladder in CLAUDE.md) and delete this injection. The CLI flag stays — it's general utility.
