...
- **[2026-05-19] Four blockers resolved — act on them**: User answered the four blocker questions surfaced after the overnight run. Apply each directive at the earliest applicable slice; promote outcomes to the relevant plan/tracker and delete this entry once acted on.
  1. **UMPy is now installed** (`pip install -e /global/homes/a/adammwea/dev/pkgs/UnitMatch/UnitMatchPy/` + `pip install mat73` to satisfy UMPy's eager `__init__` import). Verified: `UnitMatchPy.bayes_functions`, `UnitMatchPy.metric_functions`, `UnitMatchPy.param_functions`, `UnitMatchPy.utils` all import. **Note**: the actual submodule names are `bayes_functions` / `metric_functions` / `param_functions` (NOT `bayes`/`metric`/`param` as `unitlink/backends/classical.py` may have guessed). Re-do `unitlink` slice 5 with real-call tests against these names. Then slice 7 (real api.match), then `kssynth_plan` slice 9 (axon_recon recon-stage integration), then `unitmatch_phase_plan` slice 5 (enable + login-node smoke). Update `open_questions.md` to mark resolved.
  ...
  3. **`max_spikes_per_unit=None` semantic — TEST WINS**: in `pipeline/stages/reconstruct/templates/integrations/spikeinterface_extract.py`, modify `_loaded_analyzer_extensions_satisfy_requested_payload` so that when `normalized_max is None`, the function returns False if the cached waveforms count is less than the count available in `_full_waveforms` (i.e. recompute is needed to expand). Keep the existing short-circuit only when cached count == full count. This is `parallelism_post_migration_cleanup_plan.md` slice 9; resolves the 5 remaining `test_spikeinterface_extract.py` failures. Also diagnose `test_build_unit_source_payload_retries_without_compat_only_kwargs` separately (it passes `ms_before`/`ms_after`/`dtype`, so `needs_prepare=True` should already kick in — likely a separate recompute-path bug).
...
- **[2026-05-19] Env-parity guardrail added — three artifacts to keep in sync**: New guardrail `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. **The three artifacts**: `environment.yml` (conda env source-of-truth, with a `pip:` list for non-sibling pip deps), `tools/bootstrap_editable_deps.sh` (sibling editables — currently DOES NOT EXIST; tracker item TBD), and `containers/axon-recon/Dockerfile` (shifter image, ARG-driven). Every capability-touching slice updates the relevant subset in the same commit range. The loop never triggers a shifter rebuild — only the user does, and only after the corresponding `environment.yml` / `bootstrap_editable_deps.sh` / Dockerfile updates have landed.

  **Current gap (2026-05-19)**: this iteration's UMPy unblocker installed `UnitMatchPy` (editable from `/global/homes/a/adammwea/dev/pkgs/UnitMatch/UnitMatchPy/`) + `mat73` into the conda env directly, without updating the three artifacts. To close:
  - **`environment.yml`**: add `mat73` to the `pip:` list. UnitMatchPy stays OUT (it's a sibling editable; the install command is documented below).
  - **Dockerfile** (`containers/axon-recon/Dockerfile`): `mat73` is already in `UNITMATCH_RUNTIME_SPEC` (line 15) but gated behind `if [ -n "${UNITMATCH_SPEC}" ]` (line 50). For the next rebuild, EITHER set `UNITMATCH_SPEC` to a UMPy install spec (e.g. pointing at the local `/opt/axon_recon/.../UnitMatchPy` if copied in, or a pinned GitHub URL), OR move `mat73` to `AXON_RECON_RUNTIME_SPEC` (always installed). Pick the cleanest approach when rebuilding.
  - **Sibling editable install** (for UnitMatchPy): until `tools/bootstrap_editable_deps.sh` exists, run `pip install -e /global/homes/a/adammwea/dev/pkgs/UnitMatch/UnitMatchPy/ && pip install mat73` against the axon_recon conda env after any env recreation. Document in this injection until the bootstrap script lands.

  **Action required (user-only — loop never triggers shifter rebuild)**: update `environment.yml` and `Dockerfile` per above, then rebuild shifter, push, `shifterimg pull`. Once shipped, update this entry to "shifter rebuilt with UMPy+mat73 — gap closed" and verify via a shifter smoke that the UMPy import chain works inside the container. From this point onward, any slice that adds/removes/upgrades a conda dep MUST update `environment.yml` (or bootstrap script for siblings) AND post a "shifter rebuild needed: X" line under USER INJECTIONS. Read `guardrails/env_parity.md` for the full contract.
...
- **kssynth + unitlink + unitmatch_phase plans** drafted in `plans/active/`.
  ...
  - Both packages still need user-initiated `git push` to GitHub. axon_recon analysis-stage `unitmatch` phase: **slices 1-3 shipped this iteration** (commits `86672e2` slice 1 scaffold; `a18cb4b` slice 2 group discovery + path resolution in `core/unitmatch_groups.py`; `575e08b` slice 3 orchestrator invokes `unitlink.match` once per (chip, well) group with idempotent skip on subsequent group targets, output landing at `<output_root>/unitmatch/<chip>/<well>/`). Phase is still `enabled: false` in both runtime YAMLs (slice 5 flips that after a real-data smoke). Slice 4 (`--targets` group mapping) + slice 5 (enable + smoke) remain queued; slice 5 is blocked on kssynth recon-stage integration (`ks_synthesizer_package_plan.md` slice 9) producing the inputs `resolve_session_inputs` looks for.
