...
- **[2026-05-19, amended twice] Env-parity contract + unification plan + LOOP CAN REBUILD SHIFTER**: `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. `plans/active/env_install_unification_plan.md` is the destination spec — moves pip deps onto `pyproject.toml` extras (`[dev]`, `[full]`) as the single source of truth.
  ...
  **🛠 SHIFTER REBUILD PENDING (2026-05-19, after slice 6)**: `env_install_unification_plan` slices 2 + 3 + 4 + 5 + 6 + 7 have all SHIPPED. The Dockerfile now drives the container build through `pip install .[full]` instead of the per-sibling `<NAME>_SPEC` ARGs. **Build attempt history**:
   ...
   - **⭐ DIRECTIVE D (2026-05-19, AUTHORIZED — loop owns this)**: close the three "in-image" gaps (SLAy, kssynth, unitlink) so the shifter image actually runs the full pipeline. **User authorizations granted with this directive**:
     ...
     **🛑 BLOCKER on step 7 (2026-05-19 23:08 PDT) — PRIVATE-REPO CLONE AUTH IN CONTAINER BUILD**: `podman build` failed at `pip install .[full-cuda]` when pip tried to `git clone https://github.com/adamwea/kssynth.git` inside the container. `adamwea/kssynth` and `adamwea/unitlink` were both created `--private` per step 2's spec (`gh repo create … --private`); `adamwea/SLAy` happens to be PUBLIC (existed before, so SLAy's clone succeeded in the same build). The container build context has no GitHub credentials, so HTTPS clone of private repos prompts for username interactively → "fatal: could not read Username for 'https://github.com'" → pip exits 128 → build fails. Three resolution paths, user picks:
       - **(P1) Make kssynth + unitlink PUBLIC.** Simplest. `gh repo edit adamwea/kssynth --visibility public --accept-visibility-change-consequences` + same for unitlink. No code in those repos is sensitive (research package scaffolds). Loop can flip + retry the build immediately.
       - **(P2) Add a BuildKit secret token to the Dockerfile.** Inject a GH PAT with `repo` scope as a `--secret` mount during build; configure pip/git in the Dockerfile to use it for github.com. Per-build secret stays out of image layers, but Dockerfile + build invocation get a one-time edit. Token can be derived from `gh auth token` on the host.
       - **(P3) Use SSH-key forwarding.** `podman build --ssh default=$SSH_AUTH_SOCK` + pin format changed from `git+https://github.com/...` to `git+ssh://git@github.com/...`. Requires the host's SSH agent to be running with the appropriate identity loaded.
     pyproject.toml edit + the SLAy/kssynth/unitlink pushes already shipped (commits `a85a3e5`, kssynth/unitlink inaugural pushes, SLAy PR `#1`); only the container-side clone-auth piece blocks the rebuild. Switching to other unblocked slices until user picks a resolution path.
