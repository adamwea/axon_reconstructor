...
- **[2026-05-19, amended twice] Env-parity contract + unification plan + LOOP CAN REBUILD SHIFTER**: `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. `plans/active/env_install_unification_plan.md` is the destination spec — moves pip deps onto `pyproject.toml` extras (`[dev]`, `[full]`) as the single source of truth.
  ...
  **🛠 SHIFTER REBUILD PENDING (2026-05-19, after slice 6)**: `env_install_unification_plan` slices 2 + 3 + 4 + 5 + 6 + 7 have all SHIPPED. The Dockerfile now drives the container build through `pip install .[full]` instead of the per-sibling `<NAME>_SPEC` ARGs. **Build attempt history**:
   ...
   - **🛠 NEXT — PUSH IN PROGRESS (2026-05-19 21:47 PDT)**:
     1. ✅ User re-logged in to docker.io as `adammwea` (verified via `podman login --get-login docker.io`).
     2. 🔄 `podman push docker.io/adammwea/axon-recon:pipeline-v2` running in background since 21:47 PDT — 12.8 GB image, expected duration 10-30 min on NERSC network. Image ID `5f464e4e037d` confirmed still in local podman storage on pscratch graphroot.
     3. After push completes: `shifterimg pull docker:adammwea/axon-recon:pipeline-v2` — pulls into the NERSC shifter registry. Replaces previous `32638ea26b` (2026-05-18 04:57:38 build).
   ...
   - **✅ DIRECTIVE C SHIPPED (2026-05-19 21:50 PDT) — docker.io auth now persists across sessions**: the "~2hr token" the loop kept hitting wasn't token expiration — it was tmpfs cleanup. Podman's default rootless authfile lives at `${XDG_RUNTIME_DIR}/containers/auth.json` = `/run/user/$(id -u)/containers/auth.json`, which is **tmpfs (RAM-backed)** managed by systemd-logind. When all SSH sessions to a login node disconnect (or user switches login nodes — each has its own `/run/user/`), systemd garbage-collects the user runtime dir and the auth file vanishes. **Fix applied (out-of-band by Claude with user authorization)**:
     1. ✅ `~/.config/containers/auth.json` populated from the live runtime auth + `chmod 600`. (NERSC permits chmod on files you own — earlier user concern was incorrect.)
     2. ✅ `~/.bashrc` appended with `export REGISTRY_AUTH_FILE="$HOME/.config/containers/auth.json"` (idempotent grep guard). Future interactive shells use the persistent path.
     3. ✅ `~/.config/containers/containers.conf` written with `[engine] auth_file = "/global/homes/a/adammwea/.config/containers/auth.json"`. This makes raw `podman` (without env var) pick up the persistent path — belt-and-suspenders.
     4. ✅ Verified: `podman login --get-login docker.io` returns `adammwea` BOTH with `REGISTRY_AUTH_FILE` set AND with it unset (containers.conf alone is sufficient).
   - **Net effect**: future login-expired interruptions stop happening. A user-initiated `podman login docker.io` only needs to happen on actual password change. The loop's `podman login --get-login docker.io` pre-check will reliably return logged-in across session boundaries.
   - **Promote when stable**: after the next shifter rebuild round confirms no auth interruptions, promote this fix into `guardrails/env_parity.md` §"Sub-rules" as a one-line "authfile must live in $HOME/.config/containers/" rule and delete this directive.
