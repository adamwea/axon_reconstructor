...
- **[2026-05-19] Env-parity guardrail added — current gap to close**: New guardrail `guardrails/env_parity.md` locks the contract that the `axon_recon` conda env and the shifter image have equivalent capabilities except for the Kilosort+CUDA stack and NERSC/HPC/SLURM runtime plumbing. **Current gap**: `UnitMatchPy` (editable install from `/global/homes/a/adammwea/dev/pkgs/UnitMatch/UnitMatchPy/`) + `mat73` were installed into the conda env this iteration (resolves UMPy blocker for unitlink) but the shifter image does not yet have them. **Action required (user-only — loop never triggers shifter rebuild)**: add both lines to the shifter Dockerfile, rebuild, push, `shifterimg pull`. Once that lands, update this entry to "shifter rebuilt with UMPy+mat73 — gap closed" and verify via a shifter smoke that the import chain works inside the container. Also: from this point onward, any slice that adds/removes/upgrades a conda dep MUST either touch the Dockerfile in the same commit range OR post a new "shifter rebuild needed: X" line under USER INJECTIONS. Read `guardrails/env_parity.md` for the full contract.
