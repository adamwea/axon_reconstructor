...
- **kssynth + unitlink + unitmatch_phase plans** drafted in `plans/active/`.
  ...
  - Both packages still need user-initiated `git push` to GitHub. axon_recon analysis-stage `unitmatch` phase: **slice 1 (scaffold) shipped this iteration** (commit `86672e2`). The phase is wired into the analysis stage with `enabled: false` defaults in both runtime YAMLs; per-target orchestrator returns a noop marker. Slices 2-5 of `unitmatch_phase_plan.md` are queued (group discovery → unitlink.match invocation → `--targets` group mapping → enable + real-data smoke).
