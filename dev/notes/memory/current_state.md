...
- **kssynth + unitlink + unitmatch_phase plans** drafted in `plans/active/`.
  ...
  - `~/dev/pkgs/unitlink/` exists locally — slices 1 (scaffold + stub `api.match`), 2 (`core/sorter_output_reader.py` — lazy KS-folder loader), and 3 (`core/union_grid.py` — cross-session channel grid) landed. 22 tests green. Commits `81e4d3c`, `fe18c77`, `9e99bf2` in the unitlink repo. **Next**: unitlink slice 4 (`core/two_halves.py` — temporal-midpoint waveform split per UMPy expectations).
