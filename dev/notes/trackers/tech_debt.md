...
- **Status**: in-progress (slice 14c approach A — target-level skip — landed for **preprocess** in commit `ee2ec7c`; remaining stages **spikesort + reconstruct + analysis** are pending the same wire-in)
...
- **Per-stage status**:
  ...
  - `spikesort`: PENDING. Phase enumeration via `phase_plan` (list of `_SpikesortRuntimePhase`); summary path convention is `getattr(stage_config, f"{phase_label}_summary_json_relpath", None)` for most phases, with some exceptions (e.g. bombcell_label writes under `bombcell_out_dir`). Will need careful per-phase mapping.
  - `reconstruct`: PENDING. Uses shared `_run_reconstruct_substage_from_runtime`; phases include resolve_sources, templates_analyzers, axon_velocity_gtrs, plot_*, report_*, clear_templates_cache. Each phase has its own summary path convention.
  - `analysis`: PENDING. `run_analysis_from_runtime` dispatches per-phase already; the target-level skip semantically lives inside each `run_analysis_<phase>` orchestrator (compute_metrics + unitmatch). Slice 14c-analysis may end up being a no-op or a small consistency check.
