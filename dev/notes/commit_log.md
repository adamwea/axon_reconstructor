...
(T2-resolved) | refinement [opus] | USER RESOLVED T2 trusted-output triage 2026-05-21 with blanket pin: "pretty much anything in the reference data is pinned. New outputs should be identical or at least similar in shape to the current reference data." TR-000 added as blanket pin on entire reference data tree (analyzed_data/Media_Density_T5_.../). TR-CAND-001, 003, 006 promoted to TR-002 (axon_velocity_gtrs unit_0598 gtr.pkl), TR-003 (save_rec_metadata device + sample rate), TR-004 (preprocess binary integrity). Meta-rule added at top of brain/trusted_outputs.md: new outputs must match reference shape OR be a justified differential; mismatches trigger STOP-AND-ASK not silent divergence. dev/notes/TODO.md: T2 marked complete; T3 ("derive invariant assertions from pinned reference tree") becomes loop's next allowed-during-pause work + scoped concretely (file structure / numpy shapes / JSON schemas / TSV columns / count invariants).
(brain-mode + 3 components) | brain-build [opus] | Add 5 brain components per user 2026-05-21 design session: (1) brain/mode.md — active mode router (extended_autonomous / collaborative / paused) with per-mode permitted-action-sets; addresses inhibition/interference-control module. (2) brain/escalation.md — perseveration limits (N=3 same-fix, M=5 stalled-slice, K=20 stalled-plan, T=50 loop-wide-thrash) + critic-rejection limit; counters tracked in brain/notes.md; addresses flexible-updating module. (3) brain/guardrails/critic_separation.md — verifier subagent discipline: every code-shipping slice MUST spawn Explore subagent with narrow scope (diff + relevant TR-xxx + invariants) before commit; actor doesn't grade own work; addresses monitor/critic-separation module. (4) loop_prompts/collaborative.md — new "ask before doing" /loop prompt; round 1; mode-aware; question-curator + non-destructive answer-executor. (5) CLAUDE.md slice protocol amendments — step 0 mode-check; step 8 critic-separation; step 9 slice_contracts enforcement; step 15 perseveration-counter resets. CLAUDE.md pointers table + brain/README.md + brain/guardrails/README.md updated for new files. Brain pause stays in effect until user transitions ACTIVE_MODE.
(brain reasoning-consistency layer) | brain-build [opus] | Add 4 reasoning-consistency components per user 2026-05-21 design session: (1) brain/decisions.md — append-only decision-rationale log with expected-failure-mode field; backfilled with D-001..D-006 from this session (plot_recons-not-new-renderer / blanket-pin-reference / resources_profiles-before-parallelism-2 / fold-into-brain / two-mode-architecture / 4-reasoning-additions). (2) brain/glossary.md — terminology lock; ~50 project-specific terms organized by Pipeline Structure / Outputs / Trust+Verification / Process / Modes / Devices+Recordings / Pipeline Tools / Infrastructure + anti-confused-with table. (3) brain/anti_patterns.md — consolidated lessons-learned; AP-001..AP-012 derived from the 7 auto-memory feedback files + USER INJECTIONS + audit findings. (4) brain/slice_contracts.md schema addition — Prediction / Actual / Delta fields per entry; Prediction MUST be filled BEFORE actor work starts so the critic can verify outcome-matches-expectation, not just outcome-matches-spec. brain/README.md backbone subsection now 8 files (mode + 7 backbone components); reading discipline updated to glance-at-glossary / consult-anti-patterns / check-decisions when appropriate. CLAUDE.md pointers table extended with the 3 new files.
(QZ3 resolution + QZ4 surfaced) | collaborative-mode-iteration [opus, acting as loop] | USER PICKED option 1 for QZ3 (spec-first as markdown). Authored Z3-TR-000/001/002/003/004 invariant spec blocks in brain/trusted_outputs.md under new "Z3 invariant specs" section (~150 lines total). Each block: Schema / Counts / Value-range / Reconciliation invariants per pinned TR-xxx. Status: pending user review. D-007 added to brain/decisions.md. QZ4 surfaced as next gate (approve all 5 / approve some-push-back-on-others / request different format / defer to pytest-first). Pytest implementation deferred to follow-up autonomous-mode session per QZ3 option-1 framing.
(QZ4 resolved + QZ5 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 1 for QZ4 via AskUserQuestion tool: approve all 5 Z3-TR-xxx invariant specs as-written. Each block now bears `✅ APPROVED 2026-05-21` in brain/trusted_outputs.md. Z3 phase-zero step closed. D-008 added. QZ5 surfaced: "Phase zero complete — lift the pause?" (transition collaborative → extended_autonomous, OR stay collaborative to resolve more backlog first, OR lift-with-conditions). FIRST use of AskUserQuestion tool from the collaborative-mode loop flow (per prompt fix commit 746348e).
(QZ5 resolved + QZ6 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 2 for QZ5: stay collaborative, drain backlog before lifting pause. brain/mode.md ACTIVE_MODE stays `collaborative`. D-009 added. QZ6 surfaced: PRE-DIAGNOSTIC GATE 1 (radivojevic apples-to-apples comparison plan) readiness check for autonomous execution post-pause.
(QZ6 resolved + QZ7 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 1 for QZ6: PRE-DIAGNOSTIC GATE 1 (radivojevic apples-to-apples) APPROVED as-written for post-pause autonomous execution. GATE 1 section in open_questions marked APPROVED inline. D-010 added. QZ7 surfaced: "Backlog appears drained. Lift the pause now, or anything else to resolve first?"
(QZ7 resolved + QZ8 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 4 for QZ7: run proactive plan audit before lifting pause. Audit walked plans/active/*.md + spot-checked kssynth slice 5 + radivojevic kick-off trigger. 3 findings surfaced as QZ8 batched multi-select: F#2 (4 completed plans should move to plans/completed/), F#3 (kssynth slice 5 spec misses GATE 1 dependency), F#4 (radivojevic plan references stale kssynth slice numbering). D-011 added.
(QZ8 resolved + QZ9 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED multi-select [F#2 + F#3 + F#4] for QZ8. Executed: (F#2) 4 plans moved active→completed (dashboard_ui_refinement, ks_synthesizer_package, unitmatch_runner_package, env_install_unification) + env_install_unification_audit + 5 cross-reference files updated; (F#3) kssynth_recon_integration_plan slice 5 prereq dependencies section added — explicit GATE 1 dependency baked in; (F#4) radivojevic_recon_algo_plan kick-off trigger updated — "kssynth slice 9" → "kssynth_recon_integration_plan.md slice 5" with parenthetical historical note. plans/active/ shrunk 14 → 9. D-012 added. QZ9 surfaced: "All 3 audit findings resolved. Lift the pause now?"
(QZ9 resolved + QZ10 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 2 for QZ9: one more audit pass before lifting. Round-2 audit covered refs/, anti_patterns, TODO.md. 3 findings surfaced as QZ10 batched multi-select: F#5 (TODO.md stale — T3 done, T4 framing weak), F#6 (dashboard_audit cross-ref to dashboard_ui_refinement_plan stale post-F#2 move), F#7 (anti_patterns missing AP-013 collaborative-mode-cadence-bug + AP-014 question-not-surfaced-to-chat — both lessons from this session's collaborative-mode round-1 bugs). D-013 added.
(QZ10 resolved + QZ11 surfaced) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED multi-select [F#5 + F#6 + F#7] for QZ10. Executed: F#5 (TODO.md refreshed — T3 marked done; T4 expanded with lift-pause edit spec; status line refined), F#6 (dashboard_audit.md gained HISTORICAL tag + Status block noting plan moved to completed/), F#7 (anti_patterns.md gained AP-013 long-ScheduleWakeup-after-question + AP-014 question-only-to-file-not-to-chat with commit-citation sources). D-014 added. QZ11 surfaced: "Audit rounds 1+2 both drained. Lift the pause NOW?" — third attempt at the lift question; loop noting risk of more audit-pass drift if we keep surfacing audits without lifting.
(QZ11 resolved — pause lifting) | collaborative-mode-iteration [opus, acting as loop, AskUserQuestion-driven] | USER PICKED option 1 for QZ11: lift now → extended_autonomous. Loop removed PAUSE NOTICE block from loop_prompts/extended_autonomous.md (cleanup of stale content; framed as not-a-round-bump per AP-007 spirit since no new rules added). D-015 added. User to edit brain/mode.md ACTIVE_MODE collaborative → extended_autonomous then fire fresh /loop session with the autonomous prompt. First autonomous work = PRE-DIAGNOSTIC GATE 1 per D-010.
(QZ11 lift-sequence cleanup) | collaborative-mode-iteration [opus, mode-mismatch HALT] | User invoked collaborative loop prompt despite mode.md uncommitted-flipped to extended_autonomous. Loop HALTed per collaborative prompt's mode-check rule, surfaced 4-option AskUserQuestion. USER PICKED option 1 (switch — commit cleanup + end). Loop committed mode.md flip + retired stale "🛑 PAUSED — planning / refinement stage" block in current_state.md (replaced with "✅ PAUSE LIFTED — extended_autonomous resumed" summary noting phase-zero Z1-Z4 complete + PRE-DIAGNOSTIC GATE 1 as next autonomous work). Commit `1088b00`. User next action: fire fresh /loop session with loop_prompts/extended_autonomous.md.
(simplification) | brain-simplification [opus, mid-loop edit per user direction] | USER DIRECTED 2026-05-21: "the mode concept is clunky. Let's just have an extended autonomous loop prompt as we have it and a collaborative loop prompt. The looper is running now, so just implement this change aware of that." Dropped brain/mode.md router file. Mode is now implicit in which `/loop` prompt is pasted; each prompt declares permitted action set inline. D-005 marked SUPERSEDED by D-016. CLAUDE.md entry protocol step 0 + slice protocol step 0 reworded; pointers table mode row removed. brain/README.md "Mode router" section replaced with "Mode = `/loop` prompt". brain/glossary.md modes section updated. brain/guardrails/critic_separation.md mode-interaction section reworded. loop_prompts/collaborative.md + extended_autonomous.md cleaned of mode.md references. brain/current_state.md PAUSE LIFTED block updated to drop mode.md ref. brain/mode.md deleted. D-016 added.
(kssynth slice 3b PATH 2 — analyzer LOAD-path --input-root fix) | extended_autonomous-iteration [opus, code-shipping slice with critic separation] | USER PICKED PATH 2 in mode-mismatch follow-up question. Audit identified asymmetric handling: discovery side probed alternate well_out_dirs (commit f33821b) but load-side fallback recursions in load_spikeinterface_analyzers (line 2890) + iter_spikeinterface_analyzers (line 3198) preserved the ORIGINAL primary analyzer_cache_dir even when recursing at alternate well_out_dir. Fix: re-derive cache_dir relative to fallback_well_out_dir via relative_to(); fall through unchanged on ValueError. Critic flagged .expanduser() asymmetry between the two sites; fixed before commit. New test `test_load_spikeinterface_analyzers_fallback_recomputes_cache_dir_relative_to_alternate` spies on _discover_cached_analyzer_dirs to verify. 25/25 spikeinterface_extract tests green. Slice contract appended. Commit d824d4a. Unblocks kssynth slice 3b heavy smoke (queued as task #4).
(rename + new prompt) | loop-prompt-restructure [opus, mid-loop edit per user direction] | USER DIRECTED 2026-05-21: rename loop_prompts/extended_autonomous.md → autonomous_collaborative.md (the asks-on-friction prompt is collab-with-asks, not fully autonomous). DELETE loop_prompts/collaborative.md (every-iteration-question-curator was overly restrictive). CREATE new loop_prompts/extended_autonomous.md round 1 — fully autonomous, pivots-on-friction (files block in open_questions.md but doesn't pause for AskUserQuestion). Bulk sed updated path refs in CLAUDE.md / brain/decisions.md / brain/current_state.md / brain/glossary.md / brain/open_questions.md / brain/README.md / brain/guardrails/critic_separation.md. Conceptual references reworded (CLAUDE.md entry protocol step 1 + slice protocol step 0, brain/README.md mode-router section, brain/glossary.md modes entries, critic_separation.md mode-interaction). D-017 added.
(radivojevic slices 10/11/12/14 shipped in sibling repo) | autonomous_collaborative-iteration [opus, user said "implement as much as you can without me"] | Shipped 4 of 6 planned algorithm-correctness slices in radivojevic2023_recon_algo sibling repo (local commits 2e1caa7, 775177e, 58eae8d, 243d13f, dbd0881; 168→172 tests pass throughout). Slice 10 = max_distance_um_indirect 200→400μm paper fix (1-line + test). Slice 14 = recursive step 1B/1C expansion (rider chain logic); knob-by-knob unit_598 sweep shows slice 14 alone DOUBLES channel coverage 372→768. Slice 12 = separate analysis_frame_us (50μs paper-matched) from upsample_factor (10× for waveform smoothness). Slice 11 = noise_std_per_channel parameter (API + adaptive_thresholding + skeletonization all updated). Slice 11 v5 smoke yielded 0 peaks due to template-pre-spike-STD proxy being wrong units — proper raw-recording-noise still needed. Slice 13 (Stage 2 on-demand pair-averaged skeletonization) deferred — biggest architectural change. Diagnostic update filed in brain/diagnostics_to_review.md with knob-by-knob comparison table.
(radivojevic slice 13 shipped) | autonomous_collaborative-iteration [opus, user "do slice 13"] | Shipped slice 13 (Stage 2 on-demand pair/triple-averaged skeletonization, paper Fig 4B+C) in sibling repo at commit af4bdea. New helpers `skeletonize_pair_averaged` + `skeletonize_triple_averaged` in core/skeletonization.py compute skeletonize(avg(image_t, image_t+1)) on demand per link candidate, matching paper Fig 4B (vs prior code's skeletonize-union approximation). Backward-compatible via api.reconstruct's `use_pair_averaged_skeleton=False` default. multi_step_tracking.py link helpers accept optional electrical_images + noise + k; when provided, use pair/triple averaging. 168→171 tests pass (+3 slice-13 tests). End-to-end smoke on unit_598 shows direct linking dominates so skeleton-assisted gets no candidates with current dense peak detection — slice 13 effect surfaces on sparser-peak inputs. All 6 planned algorithm-correctness slices (10/11/12/13/14/15) now complete.
(radivojevic paper-alignment refactor) | extended_autonomous-iteration [opus, user "fix the nomenclature of python files... there should only be two stages...right?"] | Sibling repo `1c22138`: collapsed the misnamed 3-file split (stage_1 + stage_2 orchestrator + stage_3 trajectory) into the paper's actual 2-stage shape. DELETE `core/stage_2.py` orchestrator (vestigial after slice 13). RENAME `core/stage_3.py` → `core/stage_2.py`. Rename `Stage3Result` → `Stage2Result`. `api.reconstruct()` now returns `ReconstructionResult{stage_1, stage_2, electrical_image_grid, skipped_reason}` — no `stage_3` field, no back-compat shim. Default `use_pair_averaged_skeleton=True` (paper Fig 4B/4C). Add `Stage1Trace` (per-frame step-1 peaks + per-iteration step-2/3 chain logs) and `Stage2Trace` (per-frame-pair link groupings) — populated on every call so future plotting can hook into per-iteration / per-frame state without re-running. Test suite migrated: deleted `tests/test_stage_2.py` (orchestrator coverage subsumed by `test_electrical_image.py` + `test_skeletonization.py`), renamed `tests/test_stage_3.py` → `tests/test_stage_2.py`. 165/165 tests green. Real-data smoke on M08073/000208/well000 DIV 36 unit_598 produced SOFT-gate diagnostic plot (4×2 grid showing both stages × 3 steps each) — smoke #7 + diagnostic entry filed. Slice contract appended. Known issue (central clustering / noise underestimate) unchanged; that's axon_recon slice 11 raw-recording noise wiring (next session).
(radivojevic slices 16/17/18 + per-iteration plot) | extended_autonomous-iteration [opus, user "add those to the plan and start implementing. plot diagnostics for me to look at later."] | Sibling repo `582af64`: closed 3 paper-fidelity gaps in Stage 2 (skeletonization/trajectory) that user surfaced from re-read of paper Methods. Slice 16 = `link_peaks_skeleton_assisted` ±50% velocity filter (from direct-link median, paper "Trajectories whose velocities deviated from previously estimated values by more than 50% were discarded"). Slice 17 = `link_peaks_indirect` velocity_tolerance default 2.0 → 1.5 (paper ±50%). Slice 18 = new `PeakLink.predicted_intermediate_xy_um` populated for indirect links as midpoint (paper "predicting spatial coordinates of data for the second timeframe"). 5 new tests + 1 default-lock update = 171/171 tests green. Smoke #8 on unit_598 with v4-scale thresholds shows expected effect: skel-assisted links 12 → 0 (the ±50% velocity gate is correctly suppressing over-reaching skeleton-routed links on central-blob seeds), indirect 31 → 33, direct unchanged 991. Per-iteration Stage 2 Step 1 diagnostic plot delivered: 15 panels (one per active frame-pair iteration, Δt=50μs) showing electrodes + pair-averaged skeleton overlay + frame-t/frame-{t+1} peaks + direct links. SOFT-gate filed in diagnostics_to_review.md. Plan updated with slices 16-19 spec + sequencing. Slice contract appended.
(kssynth analyzer-cache-hit fast path) | collaborative-iteration [opus, user "lazy load the analyzers properly using the info in the artifacts. No fallbacks, just get rid of that unnecessry code."] | Root cause of 19/19-segments-failing in last kssynth run identified via end-of-log walk-back at `/pscratch/.../analyzed_data/.../logs/pipeline.log` for run `20260523T040811Z`: cached analyzers (19 dirs, ~50 GB each with templates+waveforms extensions) were loaded successfully (`segment_cached_count=19` in inventory log) but the seg-processing branch at `spikeinterface_extract.py:2628` is gated on `segments_dir.exists()` resolved against PRIMARY well_out_dir (dev_outputs/.../well000) — which has no `preprocess_outputs/preprocessed_segments/` (those live only in the analyzed_data tree). Branch skipped → analyzers list stayed empty → fallback recursion to analyzed_data → analyzed_data's cache dir didn't exist → `cached_analyzers={}` → FileNotFoundError → caught at iter line 3184 → "Per-source analyzer load failed for source=NNN_recNNNN; skipping". Wall 30 min, 885 GB disk read, 51 GB peak RSS (not OOM). The `_attach_temporary_recording_if_missing` function (line 2372) is mis-named — its only early-return condition is `analyzer.has_temporary_recording()` (False for fresh disk-loaded analyzer), so it ALWAYS overrides the cached analyzer's recording with a separately-loaded `preprocess_segments` recording. User reframe: the cached analyzer is self-sufficient (recording.json carries full lazy chain Maxwell→Center→ChannelSlice→Highpass→CommonReference back to raw H5 via 14-up relative path that resolves correctly to `/pscratch/.../raw_data/.../data.raw.h5`; templates extension is already a `(335, 200, 992)` float64 array on disk matching paper-spec 20ms window). Slice: insert cache-hit fast path in `iter_spikeinterface_analyzers` that enumerates cached analyzer dirs and yields each lazy without `_attach_temporary_recording_if_missing`, `_prepare_loaded_analyzer_with_policy`, preprocess_segments discovery, or fallback recursion. Sources NOT in cache still fall through to the complex loader (analyzers phase build path unchanged). 36/36 spikeinterface_extract + kssynth_phase tests green; 61/61 kssynth sibling + analyzers_phase_dry_run tests green. Critic separation pass (Explore subagent): yield safety, exception-fallthrough retry, empty-cache no-op, stats schema match, always-override claim all verified. Slice contract + diagnostic predictions filed. Heavy smoke (kssynth re-run on existing 19-segment cache at `dev_outputs/03_radivojevic_recon_algo/04_paper_window/...`) is the validation step; not yet run.
