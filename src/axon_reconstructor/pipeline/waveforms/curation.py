...
def _run_mea_analysis_style_curation(
    *,
    ...
    if analyzer is None:
        ...
        # Keep quality-metric params robust for short recordings.
        try:
            from .qm_config import build_quality_metrics_extension_params

            min_duration_s = float(recording.get_total_duration())
            ext_params["quality_metrics"] = build_quality_metrics_extension_params(
                min_duration_s=min_duration_s,
                logger=logger,
            )
        except Exception:
            # If anything goes wrong, fall back to SpikeInterface defaults.
            pass
    ...
    clean_metrics, rejection_log = apply_mea_analysis_curation(q_metrics=q_metrics, user_thresholds=None)
