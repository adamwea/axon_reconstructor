...
From `<well>/stg4_templates_outputs/full_channels_templates/unit_<id>/`:
...
From `<well>/stg4_templates_outputs/merged_units/unit_<id>/`:

- `merged_contributing_template_meta.json`

## Outputs (artifacts)

Under `<well>/stg5_reconstruction_outputs/`:
...
flowchart TD
  ...
  B --> C[stg5_reconstruction_outputs/reconstruction_summary.json]
  B --> D[stg5_reconstruction_outputs/all_units_morphology.pdf]
  B --> E[stg5_reconstruction_outputs/by_unit/unit_*/...]
```

## Notes

- This step uses a dedicated reconstruction checkpoint file so it can resume independently of earlier stages.
- Standalone strategy modules are available under `pipeline/stg5_reconstruction/strategies/`.
