...
def _resolve_sorter_output_dir(*, well_out_dir: Path) -> Path:
    p = Path(well_out_dir) / "spikesorting_outputs" / "sorter_output"
    if p.exists():
        return p
...
def _load_preprocessed_concat_fs_hz(*, well_out_dir: Path) -> float:
    ...
    rec_dir = Path(well_out_dir) / "preprocess_outputs" / "preprocessed_recording"
...
def compute_botm_validation_for_unit(*, inputs: BotmValidationInputs, uid: Any, logger: logging.Logger) -> dict[str, Any]:
    ...
    # Load template.
    templates_out_dir = Path(inputs.well_out_dir) / "templates_outputs"
    ...
    preprocess_out_dir = Path(inputs.well_out_dir) / "preprocess_outputs"
    ...
    recon_unit_dir = Path(inputs.well_out_dir) / "reconstruction_outputs" / "by_unit" / f"unit_{uid}"
...
def write_botm_validation_outputs(*, inputs: BotmValidationInputs, logger: Optional[logging.Logger] = None) -> dict[str, Any]:
    ...
    if inputs.unit_ids is not None:
        ...
        merged_units_dir = Path(inputs.well_out_dir) / "templates_outputs" / "templates" / "merged"
