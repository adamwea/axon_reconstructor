...
- **No downstream spike-level filtering**: spike-level exclusions belong to the waveforms stage.
  This stage only selects which unit ids are processed (by default using spikesorting-stage curation from `<well>/stg2_spikesorting_outputs/qm_unfiltered.xlsx`).
...
- `<well>/stg3_waveforms_outputs/segment_waveforms/*/`

## Outputs (artifacts)

Under `<well>/stg4_templates_outputs/`:
...
- By default, this step processes only curated units derived from spikesorting-stage quality metrics
  (from `<well>/stg2_spikesorting_outputs/qm_unfiltered.xlsx`).
