...
class ProcessingStage(Enum):
    """MEA_Analysis-compatible stage enum.

    Values mirror MEA_Analysis/IPNAnalysis/mea_analysis_routine.py.
    """
...
def _safe_write_json(path: Path, payload: dict[str, Any]) -> None:
    ...
    # Atomic-ish write: write to a temp file on the same filesystem, then rename.
...
def parse_mea_style_metadata(file_path: Path, *, stream_id: str) -> dict[str, Optional[str]]:
    """Best-effort metadata parsing, aligned with MEA_Analysis.

    Returns keys: run_id, chip_id, project, relative_pattern, date, well.
    """
...
def compute_checkpoint_file(
    *,
    ...
    """Compute a stable checkpoint file path under the per-well output directory."""
...
class CheckpointState:
    """MEA_Analysis-like JSON checkpoint state."""
...
def load_checkpoint(
    *,
    ...
    # Fresh state (mirror MEA_Analysis defaults).
...
def save_checkpoint(
    *,
    ...
    """Persist checkpoint to disk, returning the updated state."""
