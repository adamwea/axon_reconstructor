...
try:
    import dill
except ImportError:  # optional during early development/testing
    dill = None
...
# NOTE: spikeinterface (and its transitive deps like tqdm) can be heavy.
# We keep the module importable even if it isn't installed yet.
try:
...
# These are the step-specific helper libraries that implement most of the pipeline work.
# They may pull in heavier scientific dependencies, so we import them defensively.
try:
    from axon_reconstructor.internal import lib_sorting_functions as sorter
    from axon_reconstructor.internal import lib_waveform_functions as waveformer
    from axon_reconstructor.internal import mea_processing_library as MPL
    from axon_reconstructor.internal.analyze_and_reconstruct.reconstruct_and_analyze import (
        reconstruct_and_analyze,
    )
    from axon_reconstructor.internal.generate_templates import extract_templates as templater
except ImportError:  # optional during early development/testing
    sorter = None
    waveformer = None
    MPL = None
    reconstruct_and_analyze = None
    templater = None

# Legacy note: older versions imported from a top-level `modules` package via sys.path hacks.
# In this rebuild we keep everything as normal package imports under `axon_reconstructor`.
...
class AxonReconstructor:
    ...
    def setup_logger(self, prefix=None):
        ...
        if prefix is not None:
            if reconstructor_path and not os.path.exists(reconstructor_path):
                os.makedirs(reconstructor_path, exist_ok=True)
