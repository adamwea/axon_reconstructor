...
from ..runner import (
	run_spikesort_restore_sorter_output_stage,
	run_spikesort_snapshot_sorter_output_stage,
)
from .sort import _print_spikesort_aggregate, _target_datasets_override_from_args


def run_spikesort_snapshot_sorter_output(
	*,
	h5_path: Path,
	stream_id: str,
	...
	return run_spikesort_snapshot_sorter_output_stage(
		h5_path=h5_path,
		stream_id=stream_id,
...
def run_spikesort_restore_sorter_output(
	*,
	h5_path: Path,
	stream_id: str,
	mea_output_root: Path,
	...
	return run_spikesort_restore_sorter_output_stage(
		h5_path=h5_path,
		stream_id=stream_id,
		mea_output_root=mea_output_root,
		output_rel_root=output_rel_root,
...
def run_spikesort_snapshot_sorter_output_from_runtime(
	*,
	...
	from ....runner import run_spikesort_snapshot_sorter_output_from_runtime as run_runtime
...
def run_spikesort_restore_sorter_output_from_runtime(
	*,
	...
	from ....runner import run_spikesort_restore_sorter_output_from_runtime as run_runtime
...
def _run_restore_sorter_output_from_args(args: argparse.Namespace) -> int:
	if not bool(getattr(args, "confirm", False)):
		print(
			"spikesort.restore_sorter_output: refusing to overwrite canonical sorter_output without --confirm. "
			"Re-run with --confirm to overwrite each target's <well>/spikesort_outputs/sorter_output from snapshot.",
			flush=True,
		)
		return 2
	target_datasets_override = _target_datasets_override_from_args(args)
