...
LOGGER = logging.getLogger("axon_recon.init.cli")


def _emit_init_aggregate(agg: object) -> int:
	...
	for item in agg.target_results:
		...
		if isinstance(result, dict):
			LOGGER.info(
				"target[%s:%s] status=ok phase=%s init_out_dir=%s summary=%s",
				target.dataset_index,
				target.stream_id,
				result.get("phase", agg.stage),
				result.get("init_out_dir", None),
...
def _run_copy_src_to_scratch_from_args(args: argparse.Namespace) -> int:
	"""CLI handler for `axon-recon stages init.copy_src_to_scratch`."""

	from ...runner import run_init_copy_src_to_scratch_from_runtime

	config_path = str(getattr(args, "config", None) or "")
	if not config_path:
		raise SystemExit("--config is required for init.copy_src_to_scratch")
	target_datasets_override = _target_datasets_override_from_args(args)
	agg = run_init_copy_src_to_scratch_from_runtime(
	...
	return _emit_init_aggregate(agg)


def _run_from_args(args: argparse.Namespace) -> int:
	"""CLI entry point for the full init stage (`axon-recon stages init`).

	Walks `stage_config.phase_sequence` and dispatches each phase via
	`run_init_from_runtime`. With slice 5's single phase configured, this is
	equivalent to running `init.copy_src_to_scratch` end-to-end; the sequence
	abstraction lets future once-per-data-config phases slot in without a CLI
	change.
	"""
