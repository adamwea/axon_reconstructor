...
def test_build_phase_tuning_summary_recommends_active_profile_io_slot_increase_from_phase_overlap() -> None:
    runtime_config = RuntimeConfig(
        {
            "resources": {
                "active_profile": "test_profile",
...
def test_build_phase_tuning_summary_recommends_active_profile_io_slot_decrease_with_enough_observations() -> None:
    runtime_config = RuntimeConfig(
        {
            "resources": {
                "active_profile": "test_profile",
    ...
    observations = [
        {
            "timestamp": f"2026-05-04T00:00:{10 + index * 10:02d}+00:00",
            "stage": "preprocess",
            ...
            "max_threads": 1,
            "disk_read_gb": 1.0,
            "disk_read_gb_per_s": 1.0,
            "disk_write_gb": 1.0,
            "disk_write_gb_per_s": 1.0,
        }
    ...
    assert profile_recommendation["max_recommended_h5_read_slot_demand"] == 1
    assert profile_recommendation["max_recommended_disk_heavy_slot_demand"] == 1
    assert profile_recommendation["recommended_h5_read_slots"] == 1
    assert profile_recommendation["recommended_disk_heavy_slots"] == 1