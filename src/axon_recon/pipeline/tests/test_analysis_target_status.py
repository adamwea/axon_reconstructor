...
from axon_recon.pipeline.runner import run_analysis_from_runtime
...
def test_run_analysis_from_runtime_marks_target_ok(monkeypatch, tmp_path: Path) -> None:
    ...
    def _fake_parse_analysis_stage_config(**kwargs):
        return object()

    def _fake_build_analysis_inputs_for_target(*, target, stage_config, unit_workers: int, probe_pitch_um):
        _ = probe_pitch_um
        return dummy_inputs

    def _fake_run_analysis(inputs: AnalysisInputs) -> AnalysisResult:
        return AnalysisResult(
            well_out_dir=tmp_path / "well_out",
            analysis_out_dir=tmp_path / "analysis_out",
            summary_json=tmp_path / "analysis_summary.json",
            outputs={"n_units_total": "n_units_total.csv"},
        )
    ...
    monkeypatch.setattr(pipeline_runner, "parse_analysis_stage_config", _fake_parse_analysis_stage_config)
    monkeypatch.setattr(pipeline_runner, "build_analysis_inputs_for_target", _fake_build_analysis_inputs_for_target)
    monkeypatch.setattr(pipeline_runner, "run_analysis", _fake_run_analysis)

    agg = run_analysis_from_runtime(config_path=str(tmp_path / "runtime.yml"))
...
def test_run_analysis_from_runtime_marks_target_error(monkeypatch, tmp_path: Path) -> None:
    import axon_recon.pipeline.runner as pipeline_runner
    ...
    assert "analysis failed" in str(agg.target_results[0].error)
