...
from axon_recon.pipeline.runner import (
	...
	run_templates_compute_template_similarity_from_runtime,
...
def test_run_templates_compute_template_similarity_from_runtime_marks_target_ok(monkeypatch, tmp_path: Path) -> None:
	import axon_recon.pipeline.runner as pipeline_runner
	...
	def _fake_run_templates_compute_template_similarity(inputs: TemplatesInputs):
		assert inputs is dummy_inputs
		return {
			"phase": "compute_template_similarity",
			"templates_out_dir": str(tmp_path / "templates_out"),
			"summary_json": str(tmp_path / "compute_template_similarity_summary.json"),
		}
	...
	monkeypatch.setattr(
		...
		_fake_run_templates_compute_template_similarity,
	)

	agg = run_templates_compute_template_similarity_from_runtime(config_path=str(tmp_path / "runtime.yml"))

	assert agg.stage == "templates.compute_template_similarity"
	assert agg.total_targets == 1
	assert agg.succeeded_targets == 1
	assert agg.failed_targets == 0
	assert agg.target_results[0].status == "ok"
