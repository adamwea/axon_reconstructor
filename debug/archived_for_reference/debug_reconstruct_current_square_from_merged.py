...
DEFAULT_OUTPUT_JSON = REPO_ROOT / "tools" / "debug" / "outputs" / "unit94_reconstruct_current_square_from_merged.json"
DEFAULT_OUTPUT_PNG = REPO_ROOT / "tools" / "debug" / "outputs" / "unit94_reconstruct_current_square_from_merged_native.png"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Reconstruct current square_from_merged template for one unit and run AV graph tracking"
    )
    ...
    square_ch_by_t = _build_square_template(
        merged_ch_by_t,
        padding_mode="zero",
        locations_xy=merged_locs_xy,
    )
    square_locs_xy = _build_square_locations(merged_locs_xy, target_channels=int(square_ch_by_t.shape[0]))

    fs_hz = _sampling_hz_from_meta(args.target_meta)
    selected_source = "current_square_from_merged"
    ...
    payload: dict[str, Any] = {
        ...
        "template_shape_ch_by_t": list(np.asarray(square_ch_by_t).shape),
        "locs_shape": list(np.asarray(square_locs_xy).shape),
    ...
    try:
        gtr = compute_graph_tracking(
            av=av,
            template_ch_by_t=np.asarray(square_ch_by_t, dtype=float),
            locs_xy=np.asarray(square_locs_xy, dtype=float),
            sampling_frequency_hz=float(fs_hz),
        ...
        print(f"template_shape_ch_by_t={tuple(np.asarray(square_ch_by_t).shape)}")
        print(f"locs_shape={tuple(np.asarray(square_locs_xy).shape)}")
