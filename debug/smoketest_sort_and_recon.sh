...
/usr/bin/mpirun -np 2 --map-by ppr:2:node:pe=10 --bind-to core \
  axon-recon stages spikesort.bootstrap_concat_binary --config debug/debug.runtime.yml \
...
/usr/bin/mpirun -np 2 --map-by ppr:2:node:pe=10 --bind-to core \
  axon-recon stages spikesort.summarize_sort --config debug/debug.runtime.yml \
  --target-dataset 11,12 --limit-wells 1 --force-restart
