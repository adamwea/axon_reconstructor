...
LOG_DIR_DEFAULT="$ROOT_DIR/logs/multidataset_waveforms_runs"
...
usage() {
  cat <<'USAGE' >&2
Usage:
  run_multidataset_waveforms.sh [--max-parallel N] [--max-cores N] [cfg_path] [env_file] [log_dir] [-- extra_args...]

Examples:
  ./run_multidataset_waveforms.sh
  ./run_multidataset_waveforms.sh --max-parallel 3
  ./run_multidataset_waveforms.sh --max-parallel 3 --max-cores 32
  ./run_multidataset_waveforms.sh -- --force-restart
  ./run_multidataset_waveforms.sh ./cross_well_config.yml ./debug.env ./logs/waveforms -- --n-jobs 8
USAGE
}
...
# Estimate per-well n_jobs used by debug_waveforms_step.py
# Priority: --extra-args --n-jobs > AXON_RECON_N_JOBS in env file > 16 default
PER_WELL_N_JOBS=""
...
echo "Starting waveforms batch for $total targets"
echo "- cfg: $CFG_PATH"
echo "- env: $ENV_FILE"
echo "- logs: $LOG_DIR"
...
run_one() {
  ...
  echo "[$idx/$total] Waveforms $well_id :: $h5_path"
...
echo "Done waveforms batch in ${dt}s. ok=$ok fail=$fail"
if [[ $fail -gt 0 ]]; then
