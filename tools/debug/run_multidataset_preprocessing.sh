...
LOG_DIR_DEFAULT="$ROOT_DIR/logs/multidataset_preprocessing_runs"

CFG_PATH="${1:-$CFG_PATH_DEFAULT}"
ENV_FILE="${2:-$ENV_FILE_DEFAULT}"
LOG_DIR="${3:-$LOG_DIR_DEFAULT}"

# Optional: pass extra args to the underlying step script, e.g.
#   ./run_multidataset_preprocessing.sh ... -- --force-restart
EXTRA_ARGS=()
if [[ ${4:-} == "--" ]]; then
  shift 3
  shift 1
...
if [[ ! -f "$CFG_PATH" ]]; then
  ...
  echo "Usage: $0 [cfg_path] [env_file] [log_dir]" >&2
  exit 2
fi
...
echo "Starting preprocessing batch for $total targets"
echo "- cfg: $CFG_PATH"
echo "- env: $ENV_FILE"
echo "- logs: $LOG_DIR"
...
while IFS=$'\t' read -r h5_path well_id; do
  ...
  echo "[$idx/$total] Preprocessing $well_id :: $h5_path"
  ...
  cmd=(
    ...
    --h5-path "$h5_path"
    --stream-id "$well_id"
    --steps debug_preprocessing_step.py
  )
...
echo "Done preprocessing batch in ${dt}s. ok=$ok fail=$fail"
if [[ $fail -gt 0 ]]; then
