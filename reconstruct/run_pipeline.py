...
from reconstruct.axon_reconstructor import AxonReconstructor
...
def run_pipeline(h5_parent_dirs, mode = 'normal', **kwargs):
    if mode == 'normal':
        ...
        '''hacky way to run the pipeline on maxtwo plates, one well at a time to minimize size of temp data'''
        h5_files = helper.get_list_of_h5_files(h5_parent_dirs, **kwargs)
